...
import com.example.aplicacionocr.databinding.ActivityEditorTextoBinding

class EditorTextoActivity : AppCompatActivity() {

    private lateinit var binding: ActivityEditorTextoBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        binding = ActivityEditorTextoBinding.inflate(layoutInflater)
        ...
        documentoActual?.let {
            binding.etNombreDocumento.setText(it.nombre)
            binding.editorDeTexto.setHtml(it.contenido)
            //imageUrl = Uri.parse(it.imagenUri) // para no perder la imagen
        }
    ...
    private fun setListeners() {
        binding.btnDescargarE.setOnClickListener {
    ...
    private fun guardarRegistro() {
        if (documentoActual != null) {
            // Estamos EDITANDO → actualizar el documento existente
            documentoActual!!.nombre = binding.etNombreDocumento.text.toString().trim()
            documentoActual!!.contenido = binding.editorDeTexto.html ?: ""
            //documentoActual!!.imagenUri = imageUrl.toString()

            if (CrudDocumentos().update(documentoActual!!)) {
            ...
            val c = DocumentoModel(
                System.currentTimeMillis(),
                binding.etNombreDocumento.text.toString().trim(),
                //imageUrl.toString(),
                null.toString(),
                binding.editorDeTexto.html ?: "",
                "editor"
            )
